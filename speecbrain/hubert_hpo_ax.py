...
ax_client.create_experiment(
    name="emotion_recognition_hubert",
    parameters=[
        {
            "name": "lr",
            "type": "range",
            "bounds": [1e-6, 1e-4], # Adjusted upper bound for HuBERT stability
            "log_scale": True,
        },
        {
            "name": "num_epochs",
        ...
        },
        {
            "name": "max_length",
            "type": "choice",
            "values": [2 * 16000, 3 * 16000, 4 * 16000, 5 * 16000],
        },
    ],
    # Explicitly tell Ax to MAXIMIZE accuracy
    objectives={"validation_accuracy": ObjectiveProperties(minimize=False)},
)
...
# Prepare CSV output
csv_file = os.path.join(os.path.dirname(__file__), "hpo_results_ax_hubert.csv")
fieldnames = [
...
for i in range(n_trials):
    ...
    # Note: We pass positive accuracy because we set minimize=False above
...
best_val_accuracy = experiment_data["mean"].max() # Maximize accuracy
